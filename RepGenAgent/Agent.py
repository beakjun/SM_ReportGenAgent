...
class PassiveGoalCreator(object):
    ...
    def __create_prompt(self):
        template ="""
            여기서 귀하는 야구 전문가입니다. 사용자의 입력을 바탕으로 어떤 보고서를 작성할지 생각하고, 그에 맞는 제목을 생성합니다.
            [안내]

                1. 사용자의 입력을 기반으로 어떤 보고서를 작성할 지 판단합니다.
                2. 생각한 보고서명을 활용하여 description을 작성합니다.
                3. 기간에 대한 범위가 없다면 가장 최신 시즌을 고려하여 작성합니다.
...
class GoalOptimizer(object):
    ...
    def __create_prompt(self):
        template = """
        당신은 야구 관련된 리포트 작성을 위한 목표 설정 전문가입니다.
        주어진 원래목표와 주어진 문서(계획서)를 기반으로 목차별로 달성 가능한 세부적인 목표를 생성합니다.
        [원래목표]
        {query}
...
class TaskDecomposer(object):
    ...
    def __create_prompt(self):
        template = """
        주어진 목표를 콘텐츠만을 기준으로 실행 가능한 태스크 목록을 생성합니다.
        [제약사항]

        1. 다음 행동만으로 목표를 달성할 것. 절대 지정된 이외의 행동을 취하지 않습니다.
        - 사용자의 요청에 따라  DB에서 적절한 데이터를 불러와 데이터프레임으로 변환합니다.
        - 사용자 요청에 따라 DB에서 적절한 데이터를 불러와서 시각화할 수 있는 코드를 작성합니다.
        - DB외에 추가로 필요한 정보가 필요할 경우만 판단해서 인터넷 검색을 통해서 정보를 추가합니다.
        2. 각 태스크는 구체적으로 상세하게 기재하며, 단독으로 실행 및 검증 가능한 정보를 포함하며 추상적인 표현을 일절 포함하지 않습니다.
        3. 목표에 작성된 콘텐츠 수를 기준으로 태스크를 최소한으로 생성할 것.
        4. 아래와 같이 구성합니다.
        - contents_id: 콘텐츠 번호. 기존 콘텐츠 명 , type: 결과유형 [Chart, Table], contents: 태스크 내용
        5. 태스크는 실행 가능한 순서로 리스트화 합니다.
        6. 태스크를 꾸밈없이 작성합니다.
        목표: {query}
        """
        prompt = ChatPromptTemplate.from_template(template)
