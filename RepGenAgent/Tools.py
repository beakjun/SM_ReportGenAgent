    ...
    def __create_prompt(self, prompt_type:str):
        prompt = {'select_table':
                  ...
                  주어진 input을 활용하여 툴을 호출한 후 나오는 DB정보를 활용해 SELECT문을 써서 {dialect}로 정의합니다.
                    ...
                    -오직 하나의 SELECT 문만 출력. 다른 문장/주석/설명 금지.

                    -SQL은 SELECT로 시작. 코드블록(백틱) 사용 금지.

                    2.스키마와 식별자

                    -테이블명은 반드시 스키마 포함(예: doosan.tb_xxx).

                    -모든 컬럼명과 별칭은 큰따옴표("")로 감싼다.

                    3.컬럼 선택과 별칭

                    -DB에 존재하는 컬럼은 스키마의 원래 컬럼명 그대로 사용(번역/라벨 금지).

                    -집계함수나 사칙연산 등으로 만든 파생 컬럼에만 영문 snake_case 별칭 부여.

                    -단순 변환(CAST, ROUND)은 별칭을 원 컬럼명으로 유지(예: ROUND(CAST("UZR" AS NUMERIC), 2) AS "UZR").

                    4.반올림 규칙

                    -소수점이 긴 값은 CAST(... AS NUMERIC) 후 ROUND(..., 2) 적용.

                    -WHERE 사용 컬럼 제외

                    -조회 조건으로 사용한 컬럼(예: "Season", "Team")은 SELECT 목록에 포함하지 않는다.

                    5.금지 사항

                    -DDL/INSERT/UPDATE/DELETE 금지.

                    -“SQLQuery:”, “Answer:”, “Output:” 같은 접두어 금지.

                    -LIMIT은 요청이 없으면 사용하지 않는다.

                    -UNION ALL을 사용할 때는 LIMIT 사용 금지.

                    6.참조와 판단

                    -제공된 스키마 정보만 참조하여 테이블/컬럼명을 결정한다.
...
class BI(object):
    ...
    def __create_prompt(self):
        ...
        - plt.subplots을 사용할 경우 1xn 으로 나열
